...
const stripe = require("stripe")(
  "sk_test_51Mdvu1CWq9uV6YuM2iH4wZdBXlSMfexDymB6hHwpmH3J9Dm7owHNBhq4l4wawzFV9dXL3xrYGbhO74oc8OeQn5uJ00It2XDg9U"
);
...
io.on("connection", function (socket) {
    ...
    database(database_config).then(function (result) {
      if (result && result[0] && result[1]) {
        ...
        let user_found = users_array.filter(function (x) {
          return (
            (x.user === data.user || x.email === data.email) &&
            decrypt(JSON.parse(x.pass)) === data.pass
          );
        });
        if (user_found && user_found.length > 0) {
          ...
          get_extra_data().then(function (res) {
            ...
            database_config.sql =
              "UPDATE casino_user SET uuid='" +
              uuid +
              "' WHERE id=" +
              user_found[0].id +
              "; ";
            database_config.sql +=
              "INSERT INTO login_user (user_id, login_date, device, ip_address, city, country) VALUES (?, ?, ?, ?, ?, ?)";
            let payload = [
              user_found[0].id,
              timestamp,
              device,
  ...
  socket.on("signup_send", (data) => {
    database_config.sql =
      'SELECT * FROM casino_user WHERE user = "' +
      data.user +
      '" AND email = "' +
      data.email +
      '"';
    ...
    database(database_config).then(function (result) {
      if (result && result.length == 0) {
        ...
        users_array = result;
        login_user = JSON.parse(
          fs.readFileSync(path.resolve(__dirname, "./json/login.json"))
        );
        let uuid = crypto.randomBytes(20).toString("hex");
        let device = get_device(socket.request.headers); // 0 = computer, 1 = mobile, 2 = other
        ...
        get_extra_data().then(function (res) {
          ...
          database_config.sql =
            "INSERT INTO casino_user (uuid, user, email, pass, account_type, money, signup) VALUES (?, ?, ?, ?, ?, ?, ?)";
          let payload = [
            uuid,
            data.user,
            data.email,
  ...
  function updateStreak(user_found, login_user) {
    ...
    if (streak > 0) {
      if (streak % 10 === 0) {
        //each 10 days the user gets a bigger prize
        prize = 10;
      } else {
        prize = 1;
      }
  ...
  function updateMoney(user_found, payload) {
    if (user_found[0]) {
      ...
      database(database_config).then(function (data) {
        ...
        database_config.sql =
          "INSERT INTO history_user (user_id, game_name, game_id, game_type, date, status, sum) ";
      ...
      database(database_config).then(function (result) {
        if (result && result.length > 0) {
          ...
          if (user_found && user_found.length > 0) {
            ...
            let table_type = data.game.table_type
              ? data.game.table_type
              : table_name;
  ...
  socket.on("dashboardChanges_send", function (data) {
    if (data.uuid) {
      switch (data.type) {
        case "pic":
          database_config.sql =
            "UPDATE casino_user SET profile_pic='" +
            data.value +
            "' WHERE id=" +
            data.uuid +
            "; ";
